...
LANGUAGE_FILE_PATH = os.path.join("envs", "examples", "natural_language.txt")
PREDEFINED_ARGS = [
    "--config configs/train_reach.json",
    "--config configs/train_press.json",
    "--config configs/train_switch.json",
    "--config configs/train_push.json",
    "--config configs/train_poke.json",
...
def get_parser():
    ...
    parser.add_argument("-cmd", "--command", type=str, default="",
                        help="If passed and the --type is \"description\", then a description is generated based on a "
                             ...
                             "\"new_tasks\", then the argument can't be omitted")
...
def main():
    ...
    if args.type == "description":
        if args.command != '':
            ...
            print("Command generating an environment -> Natural language description:")
