    ...
    for e in range(arg_dict["eval_episodes"]):
        ...
        ##Obtaining and Saving Joint Angles
            ...
            try:
                ...
                gq = env.unwrapped.robot.get_gjoints_states()#Getting gripper angles
            except Exception:#Fallback for multi-env wrappers
                try:
                    q = env.env.robot.get_joints_states()
                    gq = env.env.robot.get_gjoints_states()
                except Exception:
                    pass
        ...
        np.save(os.path.join(save_dir, "joint_trajectory.npy"), joint_traj)  #Saving only arm joints
